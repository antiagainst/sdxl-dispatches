  ...
  hal.executable.variant public @rocm_hsaco_fb target(<"rocm", "rocm-hsaco-fb", {mma_intrinsics = [#iree_gpu.mfma_layout<F16_16x16x16_F32>, #iree_gpu.mfma_layout<F16_32x32x8_F32>], target_arch = "gfx942", ukernels = "none"}>) {
    ...
    builtin.module {
      func.func @main$async_dispatch_1334_conv_2d_nhwc_hwcf_2x128x128x320x3x3x960_f16xf16xf32() {
        ...
        %0 = hal.interface.binding.subspan set(0) binding(0) type(storage_buffer) alignment(64) offset(%c315938304) flags(ReadOnly) : !flow.dispatch.tensor<readonly:tensor<2x130x130x960xf16>>
        ...
        %13 = linalg.fill {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 64, 64, 1, 1, 32]]>} ins(%cst : f32) outs(%12 : tensor<2x128x128x320xf32>) -> tensor<2x128x128x320xf32>
        %14 = linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : vector<2xi64>, lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 64, 64, 1, 1, 32]]>, strides = dense<1> : vector<2xi64>} ins(%6, %7 : tensor<2x130x130x960xf16>, tensor<3x3x960x320xf16>) outs(%13 : tensor<2x128x128x320xf32>) -> tensor<2x128x128x320xf32>
