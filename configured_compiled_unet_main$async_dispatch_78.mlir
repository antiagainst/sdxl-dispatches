  ...
  hal.executable.variant public @rocm_hsaco_fb target(<"rocm", "rocm-hsaco-fb", {mma_intrinsics = [#iree_gpu.mfma_layout<F16_16x16x16_F32>, #iree_gpu.mfma_layout<F16_32x32x8_F32>], target_arch = "gfx942", ukernels = "none"}>) {
    ...
    builtin.module {
      func.func @main$async_dispatch_78_conv_2d_nhwc_hwcf_2x64x64x640x3x3x640_f16xf16xf32() {
        ...
        %1 = hal.interface.binding.subspan set(0) binding(1) type(storage_buffer) alignment(64) offset(%c42019840) flags(ReadOnly) : !flow.dispatch.tensor<readonly:tensor<3x3x640x640xf16>>
        ...
        %6 = linalg.fill {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 64, 128, 0, 0, 32], [0, 0, 0, 0, 1, 1, 0]]>} ins(%cst : f32) outs(%5 : tensor<2x64x64x640xf32>) -> tensor<2x64x64x640xf32>
        %7 = linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : vector<2xi64>, lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 64, 128, 0, 0, 32], [0, 0, 0, 0, 1, 1, 0]]>, strides = dense<1> : vector<2xi64>} ins(%3, %4 : tensor<2x66x66x640xf16>, tensor<3x3x640x640xf16>) outs(%6 : tensor<2x64x64x640xf32>) -> tensor<2x64x64x640xf32>
