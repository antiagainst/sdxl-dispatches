  ...
  hal.executable.variant public @rocm_hsaco_fb target(<"rocm", "rocm-hsaco-fb", {mma_intrinsics = [#iree_gpu.mfma_layout<F16_16x16x16_F32>, #iree_gpu.mfma_layout<F16_32x32x8_F32>], target_arch = "gfx942", ukernels = "none"}>) {
    ...
    builtin.module {
      func.func @main$async_dispatch_1199_conv_2d_nhwc_hwcf_2x128x128x320x3x3x320_f16xf16xf32() {
        ...
        %25 = linalg.fill {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 64, 64, 0, 0, 32], [0, 0, 0, 0, 1, 1, 0]]>} ins(%cst : f32) outs(%24 : tensor<2x128x128x320xf32>) -> tensor<2x128x128x320xf32>
        %26 = linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : vector<2xi64>, lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 64, 64, 0, 0, 32], [0, 0, 0, 0, 1, 1, 0]]>, strides = dense<1> : vector<2xi64>} ins(%18, %19 : tensor<2x130x130x320xf16>, tensor<3x3x320x320xf16>) outs(%25 : tensor<2x128x128x320xf32>) -> tensor<2x128x128x320xf32>
