hal.executable public @main$async_dispatch_1492 {
  hal.executable.variant public @rocm_hsaco_fb target(<"rocm", "rocm-hsaco-fb", {mma_intrinsics = [#iree_gpu.mfma_layout<F16_16x16x16_F32>, #iree_gpu.mfma_layout<F16_32x32x8_F32>], target_arch = "gfx942", ukernels = "none"}>) sources({compiled_unet_main$async_dispatch_1492_rocm_hsaco_fb_0_source.mlir = dense_resource<compiled_unet_main$async_dispatch_1492_rocm_hsaco_fb_0_source.mlir> : vector<3680xi8>}) {
    hal.executable.export public @main$async_dispatch_1492_generic_2x128x128x960_f32xf16xf16xf16 ordinal(0) layout(#hal.pipeline.layout<push_constants = 0, sets = [<0, bindings = [<0, storage_buffer, ReadOnly>, <1, storage_buffer, ReadOnly>, <2, storage_buffer, ReadOnly>, <3, storage_buffer>]>]>) attributes {hal.interface.bindings = [#hal.interface.binding<0, 0>, #hal.interface.binding<0, 1>, #hal.interface.binding<0, 2>, #hal.interface.binding<0, 3>], source_locs = {"0_source" = loc("compiled_unet_main$async_dispatch_1492_rocm_hsaco_fb_0_source.mlir":9:6)}, subgroup_size = 64 : index, translation_info = #iree_codegen.translation_info<LLVMGPUVectorize>, workgroup_size = [128 : index, 1 : index, 1 : index]} {
    ^bb0(%arg0: !hal.device):
      %x, %y, %z = flow.dispatch.workgroup_count_from_slice 
      hal.return %x, %y, %z : index, index, index
    }
    builtin.module {
      func.func @main$async_dispatch_1492_generic_2x128x128x960_f32xf16xf16xf16() {
        %cst = arith.constant 1.000000e+00 : f16
        %c230978880 = arith.constant 230978880 : index
        %c0 = arith.constant 0 : index
        %c356808000 = arith.constant 356808000 : index
        %0 = hal.interface.binding.subspan set(0) binding(0) type(storage_buffer) alignment(64) offset(%c230978880) flags(ReadOnly) : !flow.dispatch.tensor<readonly:tensor<2x960x128x128xf32>>
        ...
        %3 = hal.interface.binding.subspan set(0) binding(3) type(storage_buffer) alignment(64) offset(%c356808000) : !flow.dispatch.tensor<readwrite:tensor<2x130x130x960xf16>>
