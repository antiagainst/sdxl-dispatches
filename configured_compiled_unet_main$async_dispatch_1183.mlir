  ...
  hal.executable.variant public @rocm_hsaco_fb target(<"rocm", "rocm-hsaco-fb", {mma_intrinsics = [#iree_gpu.mfma_layout<F16_16x16x16_F32>, #iree_gpu.mfma_layout<F16_32x32x8_F32>], target_arch = "gfx942", ukernels = "none"}>) {
    ...
    builtin.module {
      func.func @main$async_dispatch_1183_conv_2d_nhwc_hwcf_2x64x64x1280x3x3x1280_f16xf16xf32() {
        ...
        %0 = hal.interface.binding.subspan set(0) binding(0) type(storage_buffer) alignment(64) offset(%c127194624) flags(ReadOnly) : !flow.dispatch.tensor<readonly:tensor<2x66x66x1280xf16>>
        ...
        %3 = hal.interface.binding.subspan set(0) binding(2) type(storage_buffer) alignment(64) offset(%c149497344) : !flow.dispatch.tensor<readwrite:tensor<1920x2x64x64xf16>>
        ...
        %9 = linalg.fill {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 32, 256, 1, 1, 32]]>} ins(%cst : f32) outs(%8 : tensor<2x64x64x1280xf32>) -> tensor<2x64x64x1280xf32>
        %10 = linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : vector<2xi64>, lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 32, 256, 1, 1, 32]]>, strides = dense<1> : vector<2xi64>} ins(%4, %5 : tensor<2x66x66x1280xf16>, tensor<3x3x1280x1280xf16>) outs(%9 : tensor<2x64x64x1280xf32>) -> tensor<2x64x64x1280xf32>
        %11 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d3)>, affine_map<(d0, d1, d2, d3) -> (d3, d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%10, %6 : tensor<2x64x64x1280xf32>, tensor<1280xf32>) outs(%7 : tensor<1280x2x64x64xf16>) attrs =  {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 32, 256, 1, 1, 32]]>} {
